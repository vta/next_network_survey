var VTA = VTA || {};
VTA.user_language = (navigator.languages ? navigator.languages[0] : (navigator.language || navigator.userLanguage))
  .toLowerCase();
        ...
        .done(function (data) {
          ...
          for (var i = 0; i < user_answers[survey_id].length; i++) {
            ...
            domg_data.push({
              ...
              "values": [{
                "label": "average response",
                "value-pct": 100 * ((self.SurveyAverageValues[q_id] - min) / (max - min))
              }, {
                "label": "your response",
                "value-pct": 100 * ((user_responses_indexed[q_id] - min) / (max - min))
              }]
            });
          }
...
VTA.Survey.Section.prototype.addRadioQuestion = function (o) {
  ...
  // FIXME: sort instead by order, not pk
